class Actividad < ApplicationRecord
  ...
  has_many :actividad_archivo, -> { order(indice: :asc) }, :dependent => :delete_all
  accepts_nested_attributes_for :actividad_archivo, allow_destroy: true

  has_many :actividad_opcion, -> { order(indice: :asc) }, :dependent => :delete_all
  ...
  has_many :actividad_alumno, -> { joins(:alumno).order(alumno_nombre: :asc,alumno_apellido: :asc) }, :dependent => :delete_all
