class Actividad < ApplicationRecord
  ...
  has_many :actividad_archivo, :dependent => :delete_all, -> {order("indice")}
  accepts_nested_attributes_for :actividad_archivo, allow_destroy: true

  has_many :actividad_opcion, :dependent => :delete_all, -> {order("indice")}
  ...
  has_many :actividad_alumno, :dependent => :delete_all, -> {order("nombre,apellido")}
