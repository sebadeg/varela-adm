    ...
    attributes_table do
      ...
      row "Alumnos" do 
        ...
        table_for Alumno.where("id IN (SELECT alumno_id FROM aactividad_alumnos WHERE aactividad_id=#{r.id})").order(:nombre,:apellido) do |t|
          t.column :nombre
          t.column :apellido
          t.column "Opcion" do |x| AactividadAlumno.find_by(aactividad_id: r.id, alumno_id: t.object.id).opcion end
          t.column "Fecha" do |x| AactividadAlumno.find_by(aactividad_id: r.id, alumno_id: t.object.id).fecha end
          t.column "Secretaría" do |x| AactividadAlumno.find_by(aactividad_id: r.id, alumno_id: t.object.id).secretaria end
          t.column "Mail" do |x| AactividadAlumno.find_by(aactividad_id: r.id, alumno_id: t.object.id).secretaria end
        end
      end
    end
  end
