ActiveAdmin.register Inscripcion do
  ...
  show do
    ...
    attributes_table title:"Nivel" do
      ...
      row "Formulario" do |r| ( (formulario = Convenio.find(r.formulario)) != nil ? "#{formulario.valor}%" : "") end
      row :afinidad
  ...
  form do |f|    
    ...
    f.inputs "Nivel" do
      ...
      f.input :formulario, :label => 'Formulario', :as => :select, :collection => Convenio.where("formulario").order(:valor).map{|c| ["#{c.valor}%", c.id]}
      f.input :afinidad
