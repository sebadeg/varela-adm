ActiveAdmin.register Subgrado do
  ...
  menu priority: 1023, label: 'Grados'
    ...
    attributes_table do
      ...
      row "Alumnos" do |r|
        table_for Alumno.where("id in (SELECT alumno_id FROM subgrado_alumnos WHERE subgrado_id=#{r.id})").order(:nombre, :apellido) do |t|
          ...
          t.column "Pase" do |x| (InscripcionAlumno.where( "NOT fecha_pase IS NULL" ).first rescue nil) != nil ? InscripcionAlumno.where( "alumno_id=#{x.id}" ).first.fecha_pase != nil : "" end
