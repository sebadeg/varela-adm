ActiveAdmin.register Inscripcion do
  ...
  show do
    ...
    attributes_table title:"Inscripción" do
      ...
      row "Convenio" do |r| (r.convenio != nil ? "#{r.convenio.nombre}" : "") end
      row "Adicional" do |r| (r.adicional != nil ? "#{r.adicional.nombre}" : "") end
      row "Matrícula" do |r| (r.matricula != nil ? "#{r.matricula.nombre}" : "") end
      row "Hermanos" do |r| (r.hermanos != nil ? "#{r.hermanos.nombre}" : "") end
      row "Cuotas" do |r| (r.cuotas != nil ? "#{r.cuotas.nombre}" : "") end
  ...
  form do |f|    
    ...
    f.inputs "Inscripción" do
      ...
      f.input :convenio_id, :label => 'Convenio', :as => :select, :collection => InscripcionOpcion.where( "inscripcion_opcion_tipo_id IN (SELECT id FROM InscripcionOpcionTipo WHERE nombre='Convenio' AND NOT id IS NULL)" ).order(:nombre).map{|c| ["#{c.nombre}", c.id]}
      f.input :adicional_id, :label => 'Adicional', :as => :select, :collection => InscripcionOpcion.where( "inscripcion_opcion_tipo_id IN (SELECT id FROM InscripcionOpcionTipo WHERE nombre='Adicional' AND NOT id IS NULL)" ).order(:nombre).map{|c| ["#{c.nombre}", c.id]}
      f.input :matricula_id, :label => 'Matrícula', :as => :select, :collection => InscripcionOpcion.where( "inscripcion_opcion_tipo_id IN (SELECT id FROM InscripcionOpcionTipo WHERE nombre='Matrícula' AND NOT id IS NULL)" ).order(:nombre).map{|c| ["#{c.nombre}", c.id]}
      f.input :hermanos_id, :label => 'Hermanos', :as => :select, :collection => InscripcionOpcion.where( "inscripcion_opcion_tipo_id IN (SELECT id FROM InscripcionOpcionTipo WHERE nombre='Hermanos' AND NOT id IS NULL)" ).order(:nombre).map{|c| ["#{c.nombre}", c.id]}
      f.input :cuota_id, :label => 'Cuotas', :as => :select, :collection => InscripcionOpcion.where( "inscripcion_opcion_tipo_id IN (SELECT id FROM InscripcionOpcionTipo WHERE nombre='Cuotas' AND NOT id IS NULL)" ).order(:nombre).map{|c| ["#{c.nombre}", c.id]}
    end
