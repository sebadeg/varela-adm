ActiveAdmin.register Movimiento do
  ...
  collection_action :exportar_saico do
    ...
    Movimiento.where("debe-haber>=0 AND fecha>='2019-01-01' AND rubro_id!=0").order(:fecha,:cuenta_id).each do |mov|
      sheet.row(cantidad).push "#{I18n.l(fecha, format: '%d/%m/%Y')}","VE","#{mov.cuenta_id}","D","#{mov.debe-mov.haber}"
      cantidad = cantidad+1
      sheet.row(cantidad).push "#{I18n.l(fecha, format: '%d/%m/%Y')}","VE","#{mov.rubro_id}","C","#{mov.debe-mov.haber}"
      cantidad = cantidad+1
    end
    Movimiento.where("debe-haber<0 AND fecha>='2019-01-01' AND rubro_id!=0").order(:fecha,:cuenta_id).each do |mov|
      sheet.row(cantidad).push "#{I18n.l(fecha, format: '%d/%m/%Y')}","VE","#{mov.rubro_id}","D","#{mov.haber-mov.debe}"
      cantidad = cantidad+1
      sheet.row(cantidad).push "#{I18n.l(fecha, format: '%d/%m/%Y')}","VE","#{mov.cuenta_id}","C","#{mov.haber-mov.debe}"
      cantidad = cantidad+1
    end
